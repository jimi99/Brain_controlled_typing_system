...
def sinusoidal_sample(freqs, phases, refresh_rate, flash_frames, mode='combine'):
    """
    ...
    stim_colors = np.zeros((flash_frames, 3))[:,NA,:]
    if mode == 'combine':
        conditions = inf_combine(freqs, phases)
        # stim_colors = np.zeros((flash_frames, len(conditions), 3))
        for condition in conditions:
            freq, phase = condition[0], condition[1]
            sinw = sin(2*pi*freq*time_point + pi*phase)
            stim_colors = np.concatenate((stim_colors, np.vstack((sinw, sinw, sinw)).T[:,NA,:]), axis=1)
    elif mode == 'zip':
